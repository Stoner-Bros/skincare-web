        ...
        services: [
            {
                ...
                items: ["Trị mụn chuẩn y khoa", "Trị mụn đông trùng hạ thảo", "Trị mụn lưng"],
            },
            {
...
const ServiceDetail = () => {
    ...
    return (
        <div
            className="absolute left-0 mt-2 w-[900px] bg-white shadow-lg rounded-lg border border-gray-200 flex"
            style={{ height: "400px" }}
        ...
        >
            <div className="w-1/4 bg-pink-100 p-4 rounded-l-lg flex flex-col">
                {serviceCategories.map((category) => (
                    <button
                        key={category.name}
                        className={`block w-full text-left px-3 py-2 rounded-md transition-colors ${selectedCategory.name === category.name ? "bg-pink-500 text-white" : "bg-white text-pink-500"}`}
